...
export const handlePost = (req, res) => {
  ...
  /* 🛑 STOP GAME IF ATTEMPTS ARE OVER */
  if (attemptsLeft <= 0) {
    return res.render("index", {
      success: false,
      show: true,
      attemptsLeft: 0,
      message: "Game over! Please reset the game.",
      input,
      randomNumber,
    });
  }

  /* ⬇️ Decrement only if game is active */
  attemptsLeft = attemptsLeft - 1;

  res.cookie("attemptsLeft", attemptsLeft, {
...
export const handleLogin = (req, res) => {
  ...
  if (status) {
    //if true create a sessionID for that user
